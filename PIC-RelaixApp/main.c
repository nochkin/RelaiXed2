...
// In 'debug' mode in the IDE, this application is created without
// the bootloader, and these are needed for replacement.
// If a real bootloader is present, it overrides the
// three function entries at reset, isr_hi and isr_lo.
// The bootloader skips the programming of these in its own space.

// Never provide an explicit reset: it is generated by the compiler
